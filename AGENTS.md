...
## Админ-панель (src/services/admin.service.ts + register-bot.ts)

Доступна только пользователям с `isAdmin=true`. Первые админы задаются через `ADMIN_MAX_USER_IDS` env.

| Команда/Callback | Описание |
|-----------------|----------|
| `/admin` | Открыть админ-меню (только isAdmin) |
| `adm_stats` | Статистика: юзеры, premium, выручка |
| `adm_users_{page}` | Список пользователей с пагинацией по 10 |
| `adm_user_{id}` | Карточка пользователя + транзакции |
| `adm_menu` | Вернуться в админ-меню |
